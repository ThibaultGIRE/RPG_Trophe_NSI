...
- [Contribution de l'IA au développement](#contribution-de-lia-au-développement)
...
#### Phase de menu
- `ENTRÉE` : Démarrer une nouvelle partie ou accéder au menu de chargement
...
1. **Exploration** : Le jeu commence par une phase d'exploration où le héros se déplace sur la carte
2. **Combat** : Après l'exploration, des ennemis apparaissent et le combat commence
3. **Tour du joueur** : Vous pouvez vous déplacer (5 cases) ET attaquer une fois par tour
4. **Tour ennemi** : Les ennemis se déplacent vers vous et attaquent si à portée
5. **Victoire/Défaite** : Tuez tous les ennemis pour gagner de l'expérience
6. **Progression** : Gagnez des niveaux pour augmenter vos statistiques
...
│   ├── tactical_combat.py  # Gestion du combat tactique
│   ├── combat_manager.py   # Gestionnaire de combat
│   └── action_manager.py   # Gestion des actions
├── Entities/               # Entités du jeu
│   ├── Character.py        # Classe de base des personnages
│   ├── Player_character.py # Personnage jouable
│   ├── Ennemy.py           # Ennemis
│   └── Attack.py           # Système d'attaques
├── Map/                    # Système de carte
│   └── game_map.py         # Gestion de la carte et du rendu
├── system/                 # Systèmes du jeu
│   ├── ai_controller.py    # IA des ennemis
│   ├── enemy_spawner.py    # Génération d'ennemis
│   ├── xp_system.py        # Système d'expérience
│   └── exploration_phase.py # Phase d'exploration
...
Ce projet est à but éducatif et peut être utilisé librement pour l'apprentissage.
